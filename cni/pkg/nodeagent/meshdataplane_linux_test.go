...
// fakeTrafficRuleManager 记录 EnsureHostRules/DeleteHostRules 的调用事件序列，
// 用于验证宿主机规则 reconcile 循环的行为与 Stop 时的顺序保证。
type fakeTrafficRuleManager struct {
	mu     sync.Mutex
	events []string

	ensureCh chan struct{} // 每次 EnsureHostRules 调用发出一个信号
	repaired bool
	err      error
}
...
func newReconcileTestDataplane(fakeTM *fakeTrafficRuleManager, interval time.Duration) *meshDataplane {
	// Stop(false) 会走 hostAddrSet 的 Flush/DestroySet，这里用 mock 兜住
...
func TestHostRulesReconcileLoopTicksAndStops(t *testing.T) {
	...
	// 循环应按 interval 周期性触发 EnsureHostRules
	waitForEnsureCalls(t, fakeTM, 3)

	dp.Stop(false)

	// Stop 返回后循环必须已经退出：
	// 1. 事件序列中 delete（清理）之后绝不允许再出现 ensure（否则清理后的规则会被补回）
...
func TestHostRulesReconcileLoopToleratesErrors(t *testing.T) {
	// EnsureHostRules 持续报错时循环不能退出，应在下个 tick 继续重试
...
func TestHostRulesReconcileStopIsIdempotentWithSkipCleanup(t *testing.T) {
	// skipCleanup=true（升级场景）时也必须先停循环，且不触发 DeleteHostRules
